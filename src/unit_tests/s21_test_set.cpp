...
TEST(SetTest, CountExistingElement) {
  ...
  ASSERT_EQ(set.count(1), 1UL);
  ASSERT_EQ(set.count(2), 1UL);
  ASSERT_EQ(set.count(3), 1UL);
}

TEST(SetTest, CountNonExistingElement) {
  s21::set<int> set{1, 2, 3};

  ASSERT_EQ(set.count(4), 0UL);
  ASSERT_EQ(set.count(5), 0UL);
}
...
TEST(SetTest, LowerBoundExistingElement) {
  ...
  ASSERT_NE(it, set.end()) << "Expected it != set.end() for lower_bound(2)";
  ASSERT_EQ(*it, 2) << "Expected *it == 2 for lower_bound(2)";
}
...
TEST(SetTest, LowerBoundNonExistingElement) {
  ...
  ASSERT_EQ(it, set.end()) << "Expected it == set.end() for lower_bound(4)";
}

TEST(SetTest, LowerBoundFirstElement) {
  s21::set<int> set{10, 20, 30};

  auto it = set.lower_bound(5);
  ASSERT_NE(it, set.end()) << "Expected it != set.end() for lower_bound(5)";
  ASSERT_EQ(*it, 10) << "Expected *it == 10 for lower_bound(5)";
}

TEST(SetTest, LowerBoundLastElement) {
  s21::set<int> set{10, 20, 30};

  auto it = set.lower_bound(35);
  ASSERT_EQ(it, set.end()) << "Expected it == set.end() for lower_bound(35)";
}
...
TEST(SetTest, UpperBoundExistingElement) {
  ...
  ASSERT_NE(it, set.end()) << "Expected it != set.end() for upper_bound(2)";
  ASSERT_EQ(*it, 3) << "Expected *it == 3 for upper_bound(2)";
}
...
TEST(SetTest, UpperBoundNonExistingElement) {
  ...
  ASSERT_EQ(it, set.end()) << "Expected it == set.end() for upper_bound(4)";
}

TEST(SetTest, UpperBoundFirstElement) {
  s21::set<int> set{10, 20, 30};

  auto it = set.upper_bound(5);
  ASSERT_NE(it, set.end()) << "Expected it != set.end() for upper_bound(5)";
  ASSERT_EQ(*it, 10) << "Expected *it == 10 for upper_bound(5)";
}

TEST(SetTest, UpperBoundLastElement) {
  s21::set<int> set{10, 20, 30};

  auto it = set.upper_bound(30);
  ASSERT_EQ(it, set.end()) << "Expected it == set.end() for upper_bound(30)";
}

TEST(SetTest, ExtractExistingElement) {
  s21::set<int> set{10, 20, 30};

  int val = set.extract(20);
  ASSERT_EQ(val, 20);
  ASSERT_EQ(set.count(20), 0UL);
  ASSERT_EQ(set.size(), 2UL);
}

TEST(SetTest, ExtractNonExistingElement) {
  s21::set<int> set{10, 20, 30};

  ASSERT_THROW(set.extract(40), std::out_of_range);
}

TEST(SetTest, ExtractFirstElement) {
  s21::set<int> set{10, 20, 30};

  int val = set.extract(10);
  ASSERT_EQ(val, 10);
  ASSERT_EQ(set.count(10), 0UL);
  ASSERT_EQ(set.size(), 2UL);
}

TEST(SetTest, ExtractLastElement) {
  s21::set<int> set{10, 20, 30};

  int val = set.extract(30);
  ASSERT_EQ(val, 30);
  ASSERT_EQ(set.count(30), 0UL);
  ASSERT_EQ(set.size(), 2UL);
}

TEST(SetTest, InsertManyTest1) {
  s21::set<int> my_set;
  auto results = my_set.insert_many(1, 2, 3, 2, 4, 5);

  EXPECT_EQ(results.size(), 6);
  EXPECT_EQ(results[0].second, true);   // 1
  EXPECT_EQ(results[1].second, true);   // 2
  EXPECT_EQ(results[2].second, true);   // 3
  EXPECT_EQ(results[3].second, false);  // 2 already exists
  EXPECT_EQ(results[4].second, true);   // 4
  EXPECT_EQ(results[5].second, true);   // 5

  EXPECT_EQ(my_set.size(), 5);
  EXPECT_TRUE(my_set.count(1));
  EXPECT_TRUE(my_set.count(2));
  EXPECT_TRUE(my_set.count(3));
  EXPECT_TRUE(my_set.count(4));
  EXPECT_TRUE(my_set.count(5));
}
